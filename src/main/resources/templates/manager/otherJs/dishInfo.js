			...
			buttonList[1].onclick=function(){
				...
				layer.open({
					...
					,btn1: function(index, layero){
						...
						var orderAdress = $("#orderAdress").val();
						var orderPhone = $("#orderPhone").val();

						if(orderTime.length==0||orderAdress.length==0||orderPhone.length==0){
							alert("请输入完整数据");
							return;
						}

						if (orderPhone.search(/(^1[3|4|5|7|8]\d{9}$)|(^09\d{8}$)/) == -1){
							alert("电话格式错误");
							return false;
						}
